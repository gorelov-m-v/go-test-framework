...
E2E тесты часто бывают медленными из-за IO-операций. Фреймворк использует нативные возможности Go (Goroutines) на полную мощность.
...
Чтобы убедиться, что шаги действительно выполняются параллельно, запустите тест с временными замерами:

```bash
# Тест с искусственными задержками для доказательства параллельности
