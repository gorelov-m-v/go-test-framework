...
### 6. Автоматическая кодогенерация (OpenAPI/gRPC → Go)
Написание HTTP/gRPC клиентов, моделей запросов и DSL методов вручную - это рутинная работа, которая **отнимает 40-60% времени** на старте автоматизации. Пока QA пишет boilerplate, разработка уходит вперед, и тесты всегда "догоняют".
Фреймворк включает встроенные генераторы, которые превращают **контракты** (OpenAPI спецификации, proto-файлы) в полностью готовые клиенты с типизированным DSL.

**Результат:** Старт автоматизации сдвигается **влево** - тесты можно начинать писать сразу после согласования API, не дожидаясь реализации. Одна команда `openapi-gen` или `protoc-gen-go-test` создает сотни строк кода за секунды. При изменении API регенерация занимает 10 секунд вместо часов ручной правки.

## Навигация

- [Быстрый старт: ваш первый тест за 10 минут](#быстрый-старт-ваш-первый-тест-за-10-минут)
    - [Шаг 1: Проверьте, что у вас установлено](#шаг-1-проверьте-что-у-вас-установлено)
    - [Шаг 2: Скачайте демо-проект](#шаг-2-скачайте-демо-проект)
    - [Шаг 3: Запустите сервис и инфраструктуру](#шаг-3-запустите-сервис-и-инфраструктуру)
    - [Шаг 4: Установите зависимости для тестов](#шаг-4-установите-зависимости-для-тестов)
    - [Шаг 5: Запустите тесты](#шаг-5-запустите-тесты)
    - [Шаг 6: Посмотрите красивый отчёт Allure](#шаг-6-посмотрите-красивый-отчёт-allure)
    - [Шаг 7: Остановите инфраструктуру](#шаг-7-остановите-инфраструктуру-после-работы)
    - [Обзор демо-тестов](#обзор-демо-тестов)
...
- [OpenAPI Generator](#openapi-generator)
    ...
    - [Примеры возможностей](#примеры-возможностей)
    - [Флаги командной строки](#флаги-командной-строки)
    - [Определение сервисов](#определение-сервисов)
    - [Стратегия именования](#стратегия-именования)
    - [Интеграция в тесты](#интеграция-в-тесты-1)

---

### Быстрый старт: ваш первый тест за 10 минут

Этот раздел поможет вам запустить готовый демо-проект с тестами. Не нужны глубокие знания Go - просто следуйте инструкциям шаг за шагом.
...
**1. Go (язык программирования)** - версия 1.25 или новее

Проверка:
```bash
go version
```
Должно вывести что-то вроде: `go version go1.25.3 windows/amd64`

Если Go не установлен, скачайте его с [golang.org/dl](https://golang.org/dl/) и установите.
...
**3. Docker Desktop (для запуска базы данных и Kafka)**

Проверка:
```bash
docker --version
```
Должно вывести что-то вроде: `Docker version 24.0.0`

Если Docker не установлен, скачайте с [docker.com](https://www.docker.com/products/docker-desktop)

**4. Allure (для красивых отчетов о тестах)**
...
Если Allure не установлен, следуйте [инструкции по установке](https://docs.qameta.io/allure/#_installing_a_commandline).

---

#### Шаг 2: Скачайте демо-проект

Откройте терминал (Windows: PowerShell, Mac/Linux: Terminal) и выполните команды:

```bash
# Создайте папку для проектов (если её ещё нет)
mkdir game-demo
cd game-demo

# Скачайте сервис (это REST API на Go)
git clone https://github.com/gorelov-m-v/game-service.git

# Скачайте тесты (это автотесты на фреймворке)
git clone https://github.com/gorelov-m-v/game-service-tests.git
```

 Результат: У вас должны появиться две папки:
- `game-demo/game-service/` - сам сервис
- `game-demo/game-service-tests/` - тесты для него

---

#### Шаг 3: Запустите сервис и инфраструктуру

Сервис работает с базой данных PostgreSQL и Kafka. Мы запустим всё через Docker Compose.

```bash
# Перейдите в папку с сервисом
cd game-service

# Запустите всё одной командой (сервис + база + Kafka)
docker-compose up -d
```

**Что происходит:**
- Docker скачивает образы PostgreSQL, Kafka, ZooKeeper (займёт 1-2 минуты при первом запуске)
- Поднимаются контейнеры
- Сервис запускается на `http://localhost:8080`

**Подождите 30 секунд**, чтобы все сервисы успели стартовать.
...
#### Шаг 4: Установите зависимости для тестов

Теперь перейдём к тестам. Нужно скачать все библиотеки, которые используют тесты:

```bash
# Перейдите в папку с тестами
cd ../game-service-tests

# Скачайте все зависимости (Go автоматически скачает фреймворк и библиотеки)
go mod download
```

**Что происходит:**
- Go читает файл `go.mod` (это список зависимостей)
- Скачивает фреймворк `go-test-framework` и библиотеку `allure-go`
- Сохраняет всё локально

 Результат: Зависимости скачаны, можно запускать тесты!

---

#### Шаг 5: Запустите тесты

Всё готово! Запускаем тесты:
...
**Что происходит:**
- Go ищет все файлы с `_test.go`
- Запускает все тесты по очереди
- Каждый тест делает HTTP-запросы к сервису, проверяет базу данных, проверяет Kafka
- Результаты сохраняются в папку `allure-results/`

**Вы увидите вывод:**
```
=== RUN   TestDatabaseFeaturesSuite
--- PASS: TestDatabaseFeaturesSuite (0.05s)

=== RUN   TestKafkaFeaturesSuite
--- PASS: TestKafkaFeaturesSuite (7.23s)

=== RUN   TestPlayerDeleteSuite
--- PASS: TestPlayerDeleteSuite (1.58s)

=== RUN   TestPlayerGetSuite
--- PASS: TestPlayerGetSuite (0.02s)

=== RUN   TestPlayerListSuite
--- PASS: TestPlayerListSuite (0.04s)

=== RUN   TestPlayerNegativeSuite
--- PASS: TestPlayerNegativeSuite (0.02s)

=== RUN   TestPlayerSuite
--- PASS: TestPlayerSuite (1.04s)

=== RUN   TestPlayerUpdateSuite
--- PASS: TestPlayerUpdateSuite (1.06s)

=== RUN   TestSimpleSuite
--- PASS: TestSimpleSuite (0.51s)

PASS
ok  	game-service-tests	11.610s
```

 Результат: **Все 9 test suites прошли успешно!** Это значит фреймворк работает корректно.

---

#### Шаг 6: Посмотрите красивый отчёт Allure

Тесты прошли, но давайте посмотрим детальный отчёт с шагами, запросами и ответами:
...
**Что происходит:**
- Allure генерирует HTML-отчёт из папки `allure-results/`
- Автоматически открывает его в браузере

**В отчёте вы увидите:**
-  Список всех тестов с их статусом (passed/failed)
-  Графики и статистику
-  Детали каждого теста: шаги, HTTP-запросы, ответы, SQL-запросы
-  Вложения: JSON-тела запросов/ответов

 Результат: Вы увидели, как фреймворк автоматически собирает всю информацию о прогоне!

---

#### Шаг 7: Остановите инфраструктуру (после работы)

Когда закончите тестирование, остановите все контейнеры:
...
Это освободит порты и остановит базу данных, Kafka и сервис.
...
Вы успешно:
1.  Скачали демо-проект
2.  Запустили сервис с базой данных и Kafka
3.  Установили зависимости
4.  Запустили 9 test suites (все прошли!)
5.  Посмотрели детальный Allure-отчёт

### Что дальше?

Теперь вы можете:
1. **Изучить код тестов** - откройте файлы `tests/*_test.go` и посмотрите, как написаны тесты
2. **Изменить тесты** - попробуйте добавить новую проверку в существующий тест
3. **Создать свой тест** - скопируйте любой `*_test.go` как шаблон

---

### Обзор демо-тестов

В проекте [game-service-tests](https://github.com/gorelov-m-v/game-service-tests) вы найдёте 9 test suites, каждый из которых демонстрирует конкретные возможности фреймворка. Они покрывают 100% функционала DSL.

| Test Suite | Файл | Что проверяет | Демонстрируемые фичи |
|------------|------|---------------|----------------------|
| **SimpleSuite** | [simple_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/simple_test.go) | Базовый E2E флоу: создание игрока и проверка события в Kafka | • POST запрос<br>• Kafka DSL (With, Send)<br>• AsyncStep с авто-retry |
| **PlayerSuite** | [player_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/player_test.go) | Полный E2E флоу: HTTP → DB → Kafka | • Создание ресурса (POST)<br>• Проверка в БД (Database DSL)<br>• Проверка события в Kafka<br>• Параллельные AsyncSteps (3 проверки одновременно) |
| **PlayerNegativeSuite** | [player_negative_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/player_negative_test.go) | Негативные сценарии: валидация запросов | • Table-Driven Tests (параметризация)<br>• RequestBodyMap (отправка невалидных данных)<br>• Тестирование отсутствующих полей, неверных типов, null значений |
| **PlayerGetSuite** | [player_get_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/player_get_test.go) | GET запросы с path параметрами | • GET метод<br>• PathParam (замена `{id}` в URL)<br>• GJSON пути для вложенных объектов (`meta.version`, `stats.gamesPlayed`)<br>• Header (кастомные заголовки)<br>• Обработка 404 ошибок |
| **PlayerListSuite** | [player_list_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/player_list_test.go) | GET запросы с query параметрами, работа с массивами | • QueryParam (фильтрация `?region=EU&status=active`)<br>• GJSON работа с массивами (`players.#`, `players.0.id`)<br>• Пагинация (limit, offset)<br>• Пустые массивы |
| **PlayerUpdateSuite** | [player_update_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/player_update_test.go) | PATCH запросы, мониторинг изменения состояния | • PATCH метод<br>• **AsyncStep с HTTP retry** (ключевая фича!)<br>• Polling GET до изменения статуса<br>• Множественные обновления<br>• Kafka события PLAYER_UPDATED |
| **PlayerDeleteSuite** | [player_delete_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/player_delete_test.go) | DELETE запросы, проверка удаления | • DELETE метод<br>• 204 No Content<br>• ExpectNotFound (проверка отсутствия в БД)<br>• Идемпотентность (двойное удаление)<br>• Kafka события PLAYER_DELETED |
| **DatabaseFeaturesSuite** | [database_features_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/database_features_test.go) | Все возможности Database DSL | • ExpectFound / ExpectNotFound<br>• ExpectColumnEquals / ExpectColumnNotEquals<br>• ExpectColumnNotEmpty<br>• ExpectColumnTrue / ExpectColumnFalse<br>• ExpectColumnIsNull / ExpectColumnIsNotNull<br>• Работа с nullable полями |
| **KafkaFeaturesSuite** | [kafka_features_test.go](https://github.com/gorelov-m-v/game-service-tests/blob/main/kafka_features_test.go) | Все возможности Kafka DSL | • With (фильтры с AND логикой)<br>• ExpectField / ExpectFieldNotEmpty<br>• ExpectFieldTrue / ExpectFieldFalse<br>• ExpectFieldIsNull / ExpectFieldIsNotNull<br>• Unique() / UniqueWithWindow()<br>• Работа с разными типами событий |

**Рекомендация по изучению:**

1. **Начните с SimpleSuite** - самый простой тест, показывает базовый флоу
2. **Изучите PlayerSuite** - полный E2E с параллельными проверками
3. **Посмотрите PlayerUpdateSuite** - демонстрирует ключевую фичу AsyncStep с HTTP retry
4. **Изучите DatabaseFeaturesSuite и KafkaFeaturesSuite** - полный справочник по всем методам DSL
5. **Негативные тесты** - PlayerNegativeSuite показывает Table-Driven тесты

Каждый test suite содержит подробные комментарии в коде, объясняющие, что и зачем делается.
...
Написание HTTP клиентов, моделей запросов/ответов и DSL методов вручную - это рутина, которая **замедляет старт автоматизации**. Пока QA пишет boilerplate код, разработка уходит вперёд, и тесты всегда "догоняют".

**Решение:** Генератор превращает OpenAPI спецификацию в готовые клиенты **за минуты**. Это позволяет:
- ⏱️ **Сократить время разработки тестов** с дней до часов
- ⬅️ **Shift-left testing** - для старта автоматизации нужен только контракт (OpenAPI), а не готовая реализация API
- 🔄 **Синхронизация с API** - одна команда обновляет все клиенты при изменении спецификации
- ✅ **Чистый код** - без ручных ошибок, дубликатов и несоответствий API
...
openapi-gen -module your-project openapi.json
```
...
internal/
├── client/
│   ├── auth/auth_client.go          # ✨ Сгенерировано
│   └── users/users_client.go        # ✨ Сгенерировано
└── models/
    └── http/
        ├── auth/auth_models.go      # ✨ Сгенерировано
        └── users/users_models.go    # ✨ Сгенерировано
```

### Примеры возможностей

#### 1. Автоматическое разделение на сервисы
...
**Генерируется:**
```
internal/client/auth/     ← Сервис "Auth"
internal/client/users/    ← Сервис "Users"
```
...
path: /users/change_password/{user_url}
```

**Стало (в Go):**
```go
auth.Login()           // POST /auth/login
users.ChangePassword() // PATCH /users/change_password/{user_url}
```

#### 3. Разрешение конфликтов имен

**Проблема:** Несколько методов для одного ресурса
```yaml
GET    /games
GET    /games/{id}
PUT    /games/{id}
...
**Решение:** Автоматические HTTP префиксы
```go
Games()              // GET /games
GetGames(id)         // GET /games/{id}
UpdateGames(id)      // PUT /games/{id}
DeleteGames(id)      // DELETE /games/{id}
```
...
**Генерируется:**
```go
func Status(sCtx provider.StepCtx, id string) *dsl.Call[...] {
    return dsl.NewCall[...](sCtx, httpClient).
        PATCH("/games/{id}/status").
        PathParam("id", id)
}
```

#### 5. Умная очистка имен

**Убираются:**
- HTTP суффиксы: `_post`, `_get`
- Namespace префиксы: `auth_`, `jwt_`
- Дубликаты слов
- Path параметры: `_id`, `_url`

**Пример:**
```
auth_jwt_login_auth_jwt_login_post  →  Login
change_password_users_change_password_user_url_patch  →  ChangePassword
...
Флаги:
  ...
  -models string     Путь для моделей (default: internal/models/http/{service})
  -client string     Путь для клиента (default: internal/client/{service})
  -module string     Go module name для импортов (default: "generated")
```

**Примеры:**

```bash
# Все сервисы с указанием модуля
openapi-gen -module my-project openapi.json

# Только конкретный сервис
openapi-gen -module my-project -service auth openapi.json
...
Генератор использует **теги OpenAPI** для группировки endpoints:

```yaml
paths:
  /auth/login:
    post:
      tags: ["Auth"]        # ← Определяет сервис
      
  /auth/logout:
    post:
      tags: ["Auth"]        # ← Тот же сервис
      
  /users/me:
    get:
      tags: ["Users"]       # ← Другой сервис
```

**Если тегов нет** - все endpoints попадают в один сервис с именем из `spec.info.title`.

### Стратегия именования

Генератор использует многоэтапную стратегию для создания читабельных имен:

#### Приоритет 1: Последний сегмент пути

```
POST /auth/forgot-password      →  ForgotPassword()
PATCH /users/change_password    →  ChangePassword()
GET /games/sync-status          →  SyncStatus()
```

#### Приоритет 2: Короткие имена + HTTP метод

```
GET /users/me       →  GetMe()
POST /verify        →  CreateVerify()
DELETE /items/all   →  DeleteAll()
```

#### Приоритет 3: CRUD с path параметрами
...
#### Шаг 1: Сгенерируйте клиенты

```bash
openapi-gen -module my-project openapi.json
```

#### Шаг 2: Зарегистрируйте в TestEnv
...
func (s *AuthSuite) TestLogin(t provider.T) {
    var resp *client.Response[auth.LoginResponse]

    s.Step(t, "Login user", func(sCtx provider.StepCtx) {
        resp = auth.Login(sCtx).
    ...
    s.AsyncStep(t, "Check in DB", func(sCtx provider.StepCtx) {
        users.FindByEmail(sCtx, "test@test.com").
            ExpectFound().
...
```go
// internal/client/generate.go
package client

//go:generate openapi-gen -module my-project ../../openapi.json
```
...
**Рекомендации:**
- ✅ Коммитьте `openapi.json` в репозиторий
- ✅ Регенерируйте при обновлении API
- ⚠️ Не редактируйте `*_client.go` и `*_models.go` вручную
